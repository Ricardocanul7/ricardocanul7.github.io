...
const activeClasses = "[&.router-link-active]:after:scale-x-100 [&.router-link-active]:text-slate-900 dark:[&.router-link-active]:text-white";
const exactActiveClasses = "[&.router-link-exact-active]:after:scale-x-100 [&.router-link-exact-active]:text-slate-900 dark:[&.router-link-exact-active]:text-white";

const getClasses = (exact: boolean) => `${linkClasses} ${exact ? exactActiveClasses : activeClasses}`;
</script>

<template>
    <header class="sticky top-0 z-50 bg-white/80 dark:bg-slate-900 backdrop-blur-md border-b border-slate-200 dark:border-slate-900">
        <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
            <NuxtLink :to="localePath('/')" class="text-xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2 group font-plus-jakarta-sans">
            ...
            <!-- Desktop Navigation -->
            <nav :aria-label="$t('appHeader.mainNav')" class="font-inter hidden md:flex gap-6 text-sm font-semibold text-slate-500 dark:text-slate-400">
                <NuxtLink 
            ...
            <div class="flex items-center gap-3">
                ...
                <!-- Mobile Menu Button -->
                <button 
                    class="md:hidden text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
        ...
        <nav 
            ...
            class="md:hidden border-t border-slate-200 dark:border-slate-900 bg-white dark:bg-slate-900 px-6 py-4 flex flex-col gap-4 text-center text-sm font-semibold"
        >
            <NuxtLink 
                v-for="link in navLinks" 
                :key="link.path" 
                ...
                active-class="text-slate-900 dark:text-white"
